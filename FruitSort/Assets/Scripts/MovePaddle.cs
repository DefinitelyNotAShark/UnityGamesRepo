...
//This script does:
//This script goes on:
//This script requires: the Util script (lame)
...
{ 
    ...
    private void OnTriggerEnter2D(Collider2D collision)//can delete later
    { 
        Destroy(collision.gameObject);

         //holy fuck thats a huge if statement amirite? it compares the basket and fruit state converted to strings
        if (this.gameObject.GetComponentInParent<ChangeBasket>().BState.ToString() == collision.gameObject.GetComponent<MoveFruit>().FState.ToString())
        {
            GameData.points++;//MATCH
        }
        else
        {
            PointManager.patienceValue -= 8;//NO MATCH CHNAGE LATER
        }
    }
}
